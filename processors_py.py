...
def extract_page_text(page, page_num):
    ...
    # Fallback to OCR if no meaningful text found
...
def extract_pdf_text(pdf_path: str, max_workers: int = 4) -> str:
    """Extract text from PDF using multi-threading"""
    with pdfplumber.open(pdf_path) as pdf:
        # Process pages in parallel
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(extract_page_text, page, i+1)
...
def process_report(pdf_path: str, test_names: list, batch_size: int = 50) -> Dict[str, Optional[str]]:
    ...
    text_chunks = chunk_document(report_text)
    ...
    # Fill in any missing tests (that might not be in the results dictionary)
